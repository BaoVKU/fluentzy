...
class AiService {
  ...
  Future<GenerateContentResponse> checkPronunciation({
    ...
    final prompt = '''
                ...
                Give a percentage score, feedback, and right ipa of that sentence/word.
          ''';
  ...
  Future<GenerateContentResponse> detectObject(Uint8List imageBytes) async {
    final prompt = TextPart('''
                Detect the object at center of the image and give name of that object in English.
          '''.trim());
    final image = DataPart('image/jpeg', imageBytes);
    final promptIterable = [Content.multi([prompt, image])];
    return await model.generateContent(
      promptIterable,
      generationConfig: GenerationConfig(
        responseMimeType: "application/json",
